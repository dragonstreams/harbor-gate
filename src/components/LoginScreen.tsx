...
interface LoginScreenProps {
  ...
  onPlexLogin: (serverUrl: string) => Promise<void>;
}

export function LoginScreen({ runtime, onLogin, onPlexLogin }: LoginScreenProps) {
  ...
  async function submit(event: FormEvent) {
    ...
    setError("");
    try {
      if (serverId === "plex") await onPlexLogin(serverUrl);
      else await onLogin(serverId, serverUrl, username, password);
    } catch (reason) {
        ...
        <section className="mx-auto w-full max-w-md lg:order-1 lg:mx-0">
          ...
          <div className="rounded-[2rem] border border-white/15 bg-white p-6 text-[#102a43] shadow-2xl shadow-[#06121f]/40 sm:p-9">
            ...
            <form onSubmit={submit} className="mt-8 space-y-5">
              ...
              {serverId === "plex" && <p className="rounded-xl bg-amber-50 px-4 py-3 text-xs leading-5 text-amber-800">A Plex sign-in window will open. Authorize HarborGate with the Plex Pass account that owns this server.</p>}
              {error && <p role="alert" className="rounded-xl bg-red-50 px-4 py-3 text-sm text-red-700">{error}</p>}
              <Button disabled={busy} className="h-12 w-full rounded-xl bg-[#0F9F8F] font-semibold text-white shadow-lg shadow-teal-700/15 hover:bg-[#0b887b]">{busy ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}{busy ? (serverId === "plex" ? "Waiting for Plex…" : "Connecting…") : (serverId === "plex" ? "Sign in with Plex" : "Sign in securely")}<ArrowRight className="ml-2 h-4 w-4" /></Button>
            </form>
