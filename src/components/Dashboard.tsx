...
function expirationState(user: EmbyUser) {
...
export function Dashboard({ data, onRefresh, onMutate, onLogout }: DashboardProps) {
  ...
  }), [manageable, search, filter]);
  const activeCount = manageable.filter((user) => !user.Policy?.IsDisabled).length;
  const expiringCount = manageable.filter((user) => expirationState(user).label.endsWith("d left")).length;
  ...
  return (
    <div className="min-h-screen bg-[#f4f7f9] text-[#102a43]">
      <aside className="fixed inset-y-0 left-0 z-20 hidden w-64 flex-col bg-[#0b2135] px-4 py-6 text-white lg:flex">
        <div className="flex items-center gap-3 px-2"><img src="/assets/harborgate-logo.png" alt="" className="h-11 w-11 rounded-2xl" /><div><p className="font-semibold tracking-tight">HarborGate</p><p className="text-xs text-slate-400">Emby control panel</p></div></div>
        <div className="mt-10"><Navigation /></div>
        <div className="mt-auto rounded-2xl border border-white/10 bg-white/5 p-4"><div className="flex items-center gap-2 text-xs text-slate-400"><span className="h-2 w-2 rounded-full bg-[#55d7c6] shadow-[0_0_0_4px_rgba(85,215,198,.12)]" /> Server connected</div><p className="mt-2 truncate text-xs font-medium text-slate-200">33923.brr.savethecdn.com</p></div>
      </aside>
      ...
      <main className="lg:pl-64">
        <header className="sticky top-0 z-10 border-b border-slate-200/80 bg-white/90 px-4 py-3 backdrop-blur-xl sm:px-7 lg:px-10">
          <div className="mx-auto flex max-w-7xl items-center justify-between">
            ...
            <div className="hidden items-center gap-2 text-sm text-slate-500 lg:flex"><Server className="h-4 w-4 text-[#0F9F8F]" /> Emby server <span className="rounded-full bg-emerald-50 px-2 py-1 text-xs font-semibold text-emerald-700">Online</span></div>
            <DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" className="h-11 gap-3 rounded-xl px-2"><div className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#e7f8f5] text-xs font-bold text-[#087d71]">{initials(data.adminName)}</div><div className="hidden text-left sm:block"><p className="text-sm font-semibold">{data.adminName}</p><p className="text-xs text-slate-500">Administrator</p></div><ChevronDown className="h-4 w-4 text-slate-400" /></Button></DropdownMenuTrigger><DropdownMenuContent align="end" className="w-48 rounded-xl"><DropdownMenuItem onClick={onLogout} className="rounded-lg text-rose-600"><LogOut className="mr-2 h-4 w-4" /> Sign out</DropdownMenuItem></DropdownMenuContent></DropdownMenu>
          </div>
        </header>

        <div className="mx-auto max-w-7xl px-4 py-7 sm:px-7 lg:px-10 lg:py-10">
          <section className="mb-8 flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
            <div><p className="mb-2 text-xs font-semibold uppercase tracking-[0.18em] text-[#0F9F8F]">People & access</p><h1 className="text-3xl font-semibold tracking-[-0.04em] sm:text-4xl">User profiles</h1><p className="mt-2 text-sm text-slate-500">Manage who can access your Emby media server.</p></div>
      ...
      <UserDialog open={dialogOpen} user={selectedUser} onOpenChange={setDialogOpen} onSave={saveUser} />
