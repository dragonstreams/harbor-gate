...
function expirationState(user: EmbyUser, isPlex = false) {
  if (user.Policy?.IsDisabled) return { label: "Disabled", tone: "bg-rose-50 text-rose-700 border-rose-100 dark:bg-rose-950/60 dark:text-rose-300 dark:border-rose-900" };
  if (isPlex) return { label: "Active", tone: "bg-emerald-50 text-emerald-700 border-emerald-100 dark:bg-emerald-950/60 dark:text-emerald-300 dark:border-emerald-900" };
  if (!user.expiration) return { label: "No expiry", tone: "bg-slate-100 text-slate-600 border-slate-200 dark:bg-slate-800 dark:text-slate-300 dark:border-slate-700" };
  const days = Math.ceil((new Date(user.expiration).getTime() - Date.now()) / 86_400_000);
  if (days <= 0) return { label: "Expired", tone: "bg-rose-50 text-rose-700 border-rose-100 dark:bg-rose-950/60 dark:text-rose-300 dark:border-rose-900" };
  if (days <= 7) return { label: `${days}d left`, tone: "bg-amber-50 text-amber-700 border-amber-100 dark:bg-amber-950/60 dark:text-amber-300 dark:border-amber-900" };
  return { label: "Active", tone: "bg-emerald-50 text-emerald-700 border-emerald-100 dark:bg-emerald-950/60 dark:text-emerald-300 dark:border-emerald-900" };
}
...
export function Dashboard({ data, onRefresh, onMutate, onLogout }: DashboardProps) {
  ...
  return (
    <div className="min-h-screen bg-[#f4f7f9] text-[#102a43] transition-colors dark:bg-[#091925] dark:text-slate-100">
      <aside className="fixed inset-y-0 left-0 z-20 hidden w-64 flex-col bg-[#0b2135] px-4 py-6 text-white dark:bg-[#06131e] lg:flex">
        ...
        <div className="mt-auto space-y-3"><div className="flex items-center justify-between rounded-2xl border border-white/10 bg-white/5 p-2 pl-4"><span className="text-xs font-medium text-slate-300">Appearance</span><ThemeToggle inverted /></div><div className="rounded-2xl border border-white/10 bg-white/5 p-4"><div className="flex items-center gap-2 text-xs text-slate-400"><span className={`h-2 w-2 rounded-full shadow-[0_0_0_4px_rgba(85,215,198,.12)] ${data.serverId === "emby" ? "bg-[#55d7c6]" : "bg-violet-400"}`} /> {data.serverLabel} connected</div><p className="mt-2 truncate text-xs font-medium text-slate-200">{data.serverHostname}</p></div></div>
      </aside>

      <main className="lg:pl-64">
        <header className="sticky top-0 z-10 border-b border-slate-200/80 bg-white/90 px-4 py-3 backdrop-blur-xl transition-colors dark:border-slate-700/70 dark:bg-[#0d202f]/90 sm:px-7 lg:px-10">
          <div className="mx-auto flex max-w-7xl items-center justify-between">
            <div className="flex items-center gap-3 lg:hidden"><Sheet><SheetTrigger asChild><Button variant="ghost" size="icon" className="rounded-xl"><Menu className="h-5 w-5" /></Button></SheetTrigger><SheetContent side="left" className="w-72 border-0 bg-[#0b2135] text-white dark:bg-[#06131e]"><SheetHeader className="mb-8 text-left"><SheetTitle className="flex items-center gap-3 text-white"><img src="/assets/harborgate-logo.png" alt="" className="h-10 w-10 rounded-xl" /> HarborGate</SheetTitle></SheetHeader><Navigation /><div className="mt-8 flex items-center justify-between rounded-2xl border border-white/10 bg-white/5 p-2 pl-4"><span className="text-xs font-medium text-slate-300">Appearance</span><ThemeToggle inverted /></div></SheetContent></Sheet><span className="font-semibold">{view === "instances" ? "HarborGate instances" : "User profiles"}</span></div>
            <div className="hidden items-center gap-2 text-sm text-slate-500 dark:text-slate-400 lg:flex"><Server className="h-4 w-4 text-[#0F9F8F]" /> {view === "instances" ? "Master control panel" : `${data.serverLabel} server`} <span className="rounded-full bg-emerald-50 px-2 py-1 text-xs font-semibold text-emerald-700 dark:bg-emerald-950/60 dark:text-emerald-300">Online</span></div>
            <div className="flex items-center gap-1"><div className="lg:hidden"><ThemeToggle /></div><DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" className="h-11 gap-3 rounded-xl px-2"><div className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#e7f8f5] text-xs font-bold text-[#087d71] dark:bg-teal-950 dark:text-[#55d7c6]">{initials(data.adminName)}</div><div className="hidden text-left sm:block"><p className="text-sm font-semibold">{data.adminName}</p><p className="text-xs text-slate-500 dark:text-slate-400">{data.serverLabel} administrator</p></div><ChevronDown className="h-4 w-4 text-slate-400" /></Button></DropdownMenuTrigger><DropdownMenuContent align="end" className="w-48 rounded-xl"><DropdownMenuItem onClick={onLogout} className="rounded-lg text-rose-600 dark:text-rose-400"><LogOut className="mr-2 h-4 w-4" /> Switch server</DropdownMenuItem></DropdownMenuContent></DropdownMenu></div>
          </div>
        </header>
        ...
        {view === "instances" ? <InstanceManager csrf={data.csrf} /> : <div className="mx-auto max-w-7xl px-4 py-7 sm:px-7 lg:px-10 lg:py-10">
          ...
          <section className={`mb-7 grid grid-cols-2 gap-3 ${data.serverId === "plex" ? "lg:grid-cols-3" : "lg:grid-cols-4"}`}>
            {(data.serverId === "plex" ? [{ label: "Shared users", value: manageable.length, icon: UsersRound, color: "bg-[#eaf2f8] text-[#24618b] dark:bg-sky-950/60 dark:text-sky-300" }, { label: "Access enabled", value: activeCount, icon: UserRoundCheck, color: "bg-emerald-50 text-emerald-700 dark:bg-emerald-950/60 dark:text-emerald-300" }, { label: "Access disabled", value: manageable.length - activeCount, icon: UserX, color: "bg-rose-50 text-rose-700 dark:bg-rose-950/60 dark:text-rose-300" }] : [{ label: "Total profiles", value: manageable.length, icon: UsersRound, color: "bg-[#eaf2f8] text-[#24618b] dark:bg-sky-950/60 dark:text-sky-300" }, { label: "Active", value: activeCount, icon: UserRoundCheck, color: "bg-emerald-50 text-emerald-700 dark:bg-emerald-950/60 dark:text-emerald-300" }, { label: "Expiring soon", value: expiringCount, icon: CalendarClock, color: "bg-amber-50 text-amber-700 dark:bg-amber-950/60 dark:text-amber-300" }, { label: "Disabled", value: manageable.length - activeCount, icon: UserX, color: "bg-rose-50 text-rose-700 dark:bg-rose-950/60 dark:text-rose-300" }]).map((stat) => <div key={stat.label} className="rounded-2xl border border-slate-200/80 bg-white p-4 shadow-sm shadow-slate-200/40 dark:border-slate-700/70 dark:bg-[#10283a] dark:shadow-none sm:p-5"><div className={`mb-4 flex h-9 w-9 items-center justify-center rounded-xl ${stat.color}`}><stat.icon className="h-4 w-4" /></div><p className="text-2xl font-semibold tracking-tight">{stat.value}</p><p className="mt-1 text-xs text-slate-500 dark:text-slate-400">{stat.label}</p></div>)}
          </section>
          ...
          <section className="overflow-hidden rounded-[1.5rem] border border-slate-200/80 bg-white shadow-sm shadow-slate-200/60 dark:border-slate-700/70 dark:bg-[#10283a] dark:shadow-none">
            ...
            {filtered.length === 0 ? <div className="flex flex-col items-center px-6 py-14 text-center"><img src="/assets/harborgate-empty.png" alt="No matching users" className="mb-5 h-32 w-32 object-contain" /><h3 className="font-semibold">No {data.serverId === "plex" ? "shared users" : "profiles"} found</h3><p className="mt-1 max-w-sm text-sm text-slate-500 dark:text-slate-400">Try another search or filter{data.serverId === "plex" ? ", or invite a Plex username to selected libraries." : `, or create a new ${data.serverLabel} profile.`}</p></div> : <div className="divide-y divide-slate-100 dark:divide-slate-700/70">{filtered.map((user) => {
              ...
              return <article key={user.Id} className={`group grid gap-4 p-4 transition hover:bg-slate-50/70 dark:hover:bg-slate-800/40 sm:items-center sm:px-5 ${data.serverId === "plex" ? "sm:grid-cols-[minmax(220px,1.5fr)_minmax(100px,.7fr)_minmax(110px,.75fr)_130px]" : "sm:grid-cols-[minmax(220px,1.5fr)_minmax(100px,.65fr)_minmax(110px,.7fr)_minmax(110px,.75fr)_105px_38px]"}`}>
                ...
                <div><p className="mb-1 text-[10px] font-semibold uppercase tracking-wider text-slate-400 sm:hidden">Expires</p><p className="flex items-center gap-1.5 text-sm text-slate-600 dark:text-slate-300"><Clock3 className="h-3.5 w-3.5 text-slate-400" />{formatDate(user.expiration)}</p></div>
                {data.serverId !== "plex" && <div><p className="mb-1 text-[10px] font-semibold uppercase tracking-wider text-slate-400">Connections</p><p className="text-sm font-semibold text-[#24618b] dark:text-sky-300">{connectionLimit(user, data.serverId)}</p></div>}
                <div className="flex items-center gap-2"><Switch disabled={busyId === user.Id} checked={!user.Policy?.IsDisabled} onCheckedChange={(checked) => toggleUser(user, checked)} className="data-[state=checked]:bg-[#0F9F8F]" /><span className="text-xs text-slate-500 dark:text-slate-400">Access enabled</span></div>
